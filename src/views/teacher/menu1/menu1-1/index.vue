<template>
  <div class="dashboard-container">
    ...
    <el-row :gutter="20">
      <el-col v-for="item in items" :key="item.id" :span="4" style="margin-top: 15px;">
        <div class="grid-content bg-purple">
          <el-card :body-style="{ padding: '0px' }">
            <div style="padding: 14px;">
              <p><b>{{ item.user_no }}</b></p>
              <p>群組: <b>{{ item.group == 'teacher'? item.group: `${item.user_no[0]}${item.user_no[1]}` }}</b></p>
              <p>次數: <b>{{ item.count }}</b></p>
